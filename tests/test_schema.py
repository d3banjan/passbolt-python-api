"""Tests for schema definitions and constructor."""

import unittest

from passboltapi.schema import (
    PassboltPermissionTuple,
    PassboltSecretTuple,
    PassboltUserTuple,
    PassboltOpenPgpKeyTuple,
    PassboltResourceTuple,
    PassboltResourceTypeTuple,
    constructor,
)


class TestPassboltPermissionTuple(unittest.TestCase):
    """Test PassboltPermissionTuple schema correctness."""

    def test_aco_aro_types_match_api_spec(self):
        """
        Verify aco/aro field types match Passbolt API specification.

        According to Passbolt API:
        - aco (Access Control Object): Resource or Folder - what is being accessed
        - aro (Access Request Object): User or Group - who is requesting access
        """
        # Valid permission with User accessing a Resource
        perm_data = {
            "id": "perm-123",
            "aco": "Resource",
            "aco_foreign_key": "resource-456",
            "aro": "User",
            "aro_foreign_key": "user-789",
            "type": 15,
            "created": "2024-01-01T00:00:00+00:00",
            "modified": "2024-01-02T00:00:00+00:00",
        }

        perm = constructor(PassboltPermissionTuple)(perm_data)

        self.assertEqual(perm.aco, "Resource")
        self.assertEqual(perm.aro, "User")

    def test_permission_with_group_aro(self):
        """Test permission where a Group is the access requester."""
        perm_data = {
            "id": "perm-123",
            "aco": "Folder",
            "aco_foreign_key": "folder-456",
            "aro": "Group",
            "aro_foreign_key": "group-789",
            "type": 7,
            "created": "2024-01-01T00:00:00+00:00",
            "modified": "2024-01-02T00:00:00+00:00",
        }

        perm = constructor(PassboltPermissionTuple)(perm_data)

        self.assertEqual(perm.aco, "Folder")
        self.assertEqual(perm.aro, "Group")


class TestConstructorFiltering(unittest.TestCase):
    """Test that constructor() properly filters extra fields from API responses."""

    def test_constructor_filters_extra_fields(self):
        """
        Newer Passbolt API versions return extra fields like 'secret_revision_id'
        in responses. The constructor should filter these out.
        """
        api_response_body = {
            "id": "abc123",
            "user_id": "user456",
            "resource_id": "resource789",
            "data": "encrypted_data_here",
            "created": "2024-01-01T00:00:00+00:00",
            "modified": "2024-01-02T00:00:00+00:00",
            "secret_revision_id": "rev999",  # Extra field from newer API
        }

        secret = constructor(PassboltSecretTuple)(api_response_body)

        self.assertEqual(secret.id, "abc123")
        self.assertEqual(secret.user_id, "user456")
        self.assertEqual(secret.resource_id, "resource789")
        self.assertEqual(secret.data, "encrypted_data_here")

    def test_constructor_with_filter_disabled_raises_error(self):
        """Demonstrate that with filter_fields=False, extra fields cause TypeError."""
        api_response_body = {
            "id": "abc123",
            "user_id": "user456",
            "resource_id": "resource789",
            "data": "encrypted_data_here",
            "created": "2024-01-01T00:00:00+00:00",
            "modified": "2024-01-02T00:00:00+00:00",
            "secret_revision_id": "rev999",
        }

        with self.assertRaises(TypeError) as context:
            constructor(PassboltSecretTuple, filter_fields=False)(api_response_body)

        self.assertIn("secret_revision_id", str(context.exception))

    def test_constructor_handles_list_input(self):
        """Ensure constructor handles list input correctly."""
        api_response_list = [
            {
                "id": "abc123",
                "user_id": "user456",
                "resource_id": "resource789",
                "data": "encrypted_data_1",
                "created": "2024-01-01T00:00:00+00:00",
                "modified": "2024-01-02T00:00:00+00:00",
                "secret_revision_id": "rev999",
            },
            {
                "id": "def456",
                "user_id": "user789",
                "resource_id": "resource012",
                "data": "encrypted_data_2",
                "created": "2024-01-03T00:00:00+00:00",
                "modified": "2024-01-04T00:00:00+00:00",
                "secret_revision_id": "rev888",
            },
        ]

        secrets = constructor(PassboltSecretTuple)(api_response_list)

        self.assertEqual(len(secrets), 2)
        self.assertEqual(secrets[0].id, "abc123")
        self.assertEqual(secrets[1].id, "def456")

    def test_constructor_handles_none_input(self):
        """Constructor should return None for None input."""
        result = constructor(PassboltSecretTuple)(None)
        self.assertIsNone(result)

    def test_constructor_handles_empty_list(self):
        """Constructor should return empty list for empty list input."""
        result = constructor(PassboltSecretTuple)([])
        self.assertEqual(result, [])


if __name__ == "__main__":
    unittest.main()
